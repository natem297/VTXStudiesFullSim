...
for layer_index in range(5):
    # for particle in particles.values():
        ...
        hist.SetTitle(f"Guinea Pig Layer {layer_index + 1} Raw Hits;Cosine Theta; Azimuthal Angle (deg)")
        ...
        for cos_th in range(-100, 100, 2):
            ...
            for azimuthal in range(0, 360, 3):
                ...
                hist.SetBinContent((cos_th // 2) + 51, (azimuthal // 3) + 1, hits)

        hist.SetStats(0)
        canvas = ROOT.TCanvas("hit map", f"Guinea Pig Layer {layer_index + 1} Raw Hits")
        canvas.SetRightMargin(0.12)
        hist.Draw("colz")
        canvas.Update()
        # canvas.SaveAs(f"../plots/hit_rates/gp_layer{layer_index + 1}_{particle}_hit_rate_test.png")
        canvas.SaveAs(f"../plots/hit_rates/raw_hits/gp_layer{layer_index + 1}_raw_hit_rate.png")
